...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('html2js', 'Compiles Angular-JS templates to JavaScript.', function() {
    ...
    this.files.forEach(function(f) {
      ...
      var bundle = "var angular = require('angular'); \n";
      ...
      if (options.singleModule) {
        if (options.target == 'js') {
          bundle += "module.exports = angular.module('" + targetModule + "', []).run(['$templateCache', function($templateCache) {\n";
          modules += '\n}]);\n';
        } else if (options.target == 'coffee') {
          bundle += "module.exports =angular.module('" + targetModule + "', []).run(['$templateCache', ($templateCache) ->\n";
          modules += '\n])\n';
        }

      } else if (targetModule) {
        //Allow a 'no targetModule if module is null' option
        bundle += "module.exports =  angular.module('" + targetModule + "', [" + moduleNames.join(', ') + "])";
        if (options.target === 'js') {
          bundle += ';';
        }
